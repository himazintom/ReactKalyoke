...
export const PitchPlayer = () => {
  ...
  const initializeAudio = async (path) => {
    ...
    if (isPlaying) {
      ...
      if (instAudioRef.current) instAudioRef.current.stop();
      if (vocalAudioRef.current) vocalAudioRef.current.stop();
      setIsPlaying(false);
    }

    // Waveform描画用のURLはそのまま
    ...
    try {
      await Promise.all([
        new Promise((resolve) => { originalAudio.addEventListener('canplaythrough', resolve, { once: true }); }),
        instPlayer.load(`${folderPath}/no_vocals.mp3`),
        vocalPlayer.load(`${folderPath}/vocals.mp3`),
      ]);
    } catch (error) {
      console.error('オーディオファイルの読み込み中にエラーが発生しました:', error);
    }
  ...
  const handlePlayPause = (event) => {//再生・一時停止ボタンが押されたら...
    ...
    if (isPlaying) {//再生中だったら止める
      ...
      playerRef.current.playVideo();
      instAudioRef.current.start(undefined, time);
      vocalAudioRef.current.start(undefined, time);
      syncSeekOfMusicAndYoutubeApi();
    }
  ...
  const calculateVolume = (value) => {//value(0~100)
    const calculateValue = Math.max(0.0, Math.min(value, 100.0)) / 100.0
    return calculateValue ** 2; //(0.0~1.0)
  };
  ...
  const seekChange = (value) =>{//特定の位置にシークさせる
    if(prepareKaraokeStatus == 0){
      ...
      instAudioRef.current.seek(value);
      vocalAudioRef.current.seek(value);
  ...
  const syncSeekOfMusicAndYoutubeApi = () => {
    setTimeout(() => {
      ...
      const timeDifference = Math.abs(originalAudioRef.current.currentTime - playerRef.current.getCurrentTime());
    
      // ずれが0.1秒以上あれば修正
      if (timeDifference > 0.1) {
        playerRef.current.seekTo(originalAudioRef.current.currentTime);
      }
    }, 1000);  // 1秒後にチェック
  }
    ...
    const handleFocus = () => {
      setTimeout(() => {
        ...
        if (isKaraokeReady && isPlaying) { // 音楽を流していたら
          playerRef.current.seekTo(originalAudioRef.current.currentTime); // youtubeAPI(映像)の位置を現在音が再生されているところにする
        }
      }, 500); // 500ms遅延
    };
  ...
  const handleEndedMusic = useCallback(async () => {//一度しか関数が生成できないようにする
    ...
    if (isLooping) {
      ...
      if (videoData && videoData.length > 0) {
        ...
        handlePitchChange(-currentPitch);
